...
export default function AuctionDetail() {
  ...
  const fetchAuction = async () => {
    ...
    } catch (e: any) { if (e?.response?.status === 401) { localStorage.removeItem('token'); navigate('/login') } else { toast.error('Failed to load auction'); navigate('/auctions') } } finally { setLoading(false) }
  }
  ...
  const handleToggleRegistration = async () => {
    try {
      const data = await toggleRegistration(Number(id))
      setAuction({ ...auction, registration_open: data.registration_open ? 1 : 0 })
      toast.success(data.registration_open ? 'Registration opened' : 'Registration closed')
    } catch { toast.error('Failed to toggle registration') }
  }
  ...
  const regLink = `${window.location.origin}/register/${id}`
  const regOpen = !!auction.registration_open
    ...
    <>
      <div className="animate-fade-in noise-bg relative">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="font-display text-3xl md:text-5xl tracking-wider gradient-text">{auction.name}</h1>
        </div>
        ...
        <div className="glass-strong rounded-2xl p-6 mb-6">
          <div className="flex items-center justify-between mb-0">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={() => navigate(`/auctions/${id}/teams`)} title="Teams" aria-label="Teams" className="p-2.5 hover:bg-white/5 rounded-xl transition-colors"><Users className="w-5 h-5 text-gray-500" /></button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="glass-strong rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleToggleRegistration} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${regOpen ? 'bg-accent-gold' : 'bg-surface-4'}`}>
              ...
              {regOpen && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="overflow-hidden">
                  <div className="flex items-center gap-2 bg-surface-2 rounded-xl px-3 py-2 border border-white/5 text-sm text-gray-400">
                    <Link className="w-4 h-4 shrink-0" />
                    <span className="truncate font-mono text-xs">{regLink}</span>
                    <button onClick={() => copyLink(regLink)} className="ml-auto shrink-0 text-accent-gold hover:text-amber-400 transition-colors text-xs font-medium">Copy</button>
                  </div>
                  {auction.registration_deadline && (
                    <p className="text-xs text-amber-400 mt-2">Deadline: {new Date(auction.registration_deadline).toLocaleString()}</p>
                  )}
                </motion.div>
              )}
            </div>
          ...
          <div className="glass-strong rounded-2xl p-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex flex-wrap items-center gap-2"><Timer className="w-4 h-4 text-gray-500" /><p className="text-sm text-gray-400">Timer</p></div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-wrap items-center gap-2"><TrendingUp className="w-4 h-4 text-gray-500" /><p className="text-sm text-gray-400">Bid Increments</p></div>
          ...
          <div className="bg-gradient-to-br from-purple-700 to-indigo-800 rounded-2xl p-6 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-purple-200 text-sm mb-4">Choose colors, fonts, sounds, and branding for your live auction room.</p>
              <button onClick={() => navigate(`/auctions/${id}/settings`)} className="bg-white text-purple-700 px-4 py-2 rounded-xl text-sm font-bold hover:bg-purple-50 transition-colors">Open Settings</button>
            </div>
          </div>
          ...
          <div className="glass-strong rounded-2xl p-6">
            ...
            <div className="space-y-3">
              <button onClick={() => navigate(`/auctions/${id}/players/import`)} className="w-full text-left px-4 py-3 rounded-xl border border-white/5 bg-surface-2/50 hover:bg-surface-3/50 transition-colors text-sm text-gray-400">Import players from spreadsheet</button>
              <button onClick={() => navigate(`/auctions/${id}/players/stats-import`)} className="w-full text-left px-4 py-3 rounded-xl border border-white/5 bg-surface-2/50 hover:bg-surface-3/50 transition-colors text-sm text-gray-400">Import player stats</button>
            </div>
          </div>
        </div>
