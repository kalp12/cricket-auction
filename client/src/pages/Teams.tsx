          ...
          >
            <Card>
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={handleCancel} className="w-8 h-8 rounded-lg glass flex items-center justify-center text-gray-400 hover:text-accent-rose transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>

              {error && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-accent-rose/10 border border-accent-rose/20 text-accent-rose p-3 rounded-xl text-sm mb-5">
                  {error}
                </motion.div>
              )}

              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <Input label="Team Name *" placeholder="e.g. Mumbai Indians" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
                <Input label="Short Name" placeholder="e.g. MI" value={form.short_name} onChange={e => setForm({ ...form, short_name: e.target.value.toUpperCase() })} maxLength={5} />
                <Input label="Total Budget" type="number" value={form.total_budget} onChange={e => setForm({ ...form, total_budget: Number(e.target.value) })} hint={formatPrice(form.total_budget)} />
                <Input label="Max Players" type="number" value={form.max_players} onChange={e => setForm({ ...form, max_players: Number(e.target.value) })} />
                <Input label="Logo URL" placeholder="https://..." value={form.logo_url} onChange={e => setForm({ ...form, logo_url: e.target.value })} />
                <div className="flex items-end gap-3">
                  <Button variant="ghost" type="button" onClick={handleCancel}>Cancel</Button>
                  <Button variant="emerald" type="submit" disabled={saving}>{saving ? 'Saving...' : editId ? 'Update' : 'Add Team'}</Button>
                </div>
              </form>
            </Card>
          </motion.div>
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-5">
                        ...
                        {hasLogo ? (
                          <div className="w-12 h-12 rounded-xl overflow-hidden border border-white/10 shadow-lg">
                            ...
                            <div className="hidden w-full h-full bg-gradient-to-br from-accent-gold/30 to-primary-600/30 flex items-center justify-center">
                              <span className="font-display text-lg text-accent-gold">{(team.short_name || team.name)[0]}</span>
                            </div>
                          </div>
                        ) : (
                          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-accent-gold/20 to-primary-600/20 border border-accent-gold/20 flex items-center justify-center shadow-lg">
                            <span className="font-display text-xl text-accent-gold">{(team.short_name || team.name)[0]}</span>
                          </div>
                        )}
                        <div>
                          <h3 className="font-bold text-white text-lg leading-tight">{team.name}</h3>
                          <div className="flex items-center gap-2 mt-0.5">
                            {team.short_name && <Badge color="gold">{team.short_name}</Badge>}
                      ...
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="sm" icon={<Pencil className="w-3.5 h-3.5" />} onClick={() => handleEdit(team)} className="w-8 h-8 !px-0 !py-0" />
                        <Button variant="ghost" size="sm" icon={<Trash2 className="w-3.5 h-3.5" />} onClick={() => handleDelete(team.id)} className="w-8 h-8 !px-0 !py-0 !text-gray-500 hover:!text-rose-400" />
                        <Button variant="ghost" size="sm" onClick={() => toggleExpand(team.id)} className="w-8 h-8 !px-0 !py-0 ml-1">
                    ...
                    <div className="mb-3">
                      <div className="flex justify-between text-xs mb-2">
                        <span className="text-gray-400">Spent: <span className="text-accent-gold font-medium">{formatPrice(spent)}</span></span>
                        <span className="text-gray-500">Remaining: <span className="text-emerald-400 font-medium">{formatPrice(team.remaining_budget)}</span></span>
                      </div>
                      <div className="h-2.5 bg-surface-4 rounded-full overflow-hidden">
                        <motion.div
                          ...
                          className={`h-full rounded-full ${pct > 90 ? 'bg-gradient-to-r from-rose-600 to-rose-500' : pct > 70 ? 'bg-gradient-to-r from-amber-600 to-amber-500' : 'bg-gradient-to-r from-accent-gold to-emerald-400'}`}
                        />
                      </div>
                      <div className="flex justify-between mt-1.5">
                        <span className="text-[10px] text-gray-600">Budget: {formatPrice(team.total_budget)}</span>
                        <span className={`text-[10px] font-mono font-semibold ${pct > 90 ? 'text-rose-400' : pct > 70 ? 'text-amber-400' : 'text-accent-gold'}`}>{pct}% used</span>
                      </div>
                    </div>
                              ...
                              {roster.map((p: any, i: number) => (
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2">
                                    {p.role && <Badge color="gray">{p.role}</Badge>}
                                    <span className="text-sm text-accent-gold font-semibold font-mono">{formatPrice(p.bought_price)}</span>
                                  </div>
                                </motion.div>
                              ))}
                            </div>
