...
export default function CommandPalette() {
  ...
  const commands: CommandItem[] = [
    ...
    { id: 'teams', label: 'Manage Teams', sublabel: 'Team management', icon: Users, action: () => { navigate('/auction-panel'); close() }, keywords: ['team', 'manage', 'players', 'roster'] },
    { id: 'players', label: 'Manage Players', sublabel: 'Player management', icon: User, action: () => { navigate('/auction-panel'); close() }, keywords: ['player', 'manage', 'add', 'cricket'] },
    { id: 'settings', label: 'Settings', sublabel: 'Auction rules & configs', icon: Settings, action: () => { navigate('/auction-panel'); close() }, keywords: ['settings', 'config', 'rules', 'slabs', 'timer'] },
    { id: 'stats', label: 'Auction Stats', sublabel: 'Analytics & charts', icon: BarChart3, action: () => { navigate('/auction-panel'); close() }, keywords: ['stats', 'analytics', 'charts', 'data'] },
    { id: 'history', label: 'Auction History', sublabel: 'Past results', icon: Clock, action: () => { navigate('/auction-panel'); close() }, keywords: ['history', 'results', 'past', 'sold'] },
  ]
        ...
        <>
          ...
          >
            <div className="glass-strong rounded-2xl overflow-hidden shadow-2xl shadow-black/40">
              ...
              {/* Results */}
              <div className="max-h-72 overflow-y-auto p-2 dark-scrollbar">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((item, i) => (
                    ...
                    >
                      ...
                      {i === selectedIndex && <ArrowRight className="w-4 h-4 text-gray-500 shrink-0" />}
                    </button>
                  ))
                )}
              </div>
